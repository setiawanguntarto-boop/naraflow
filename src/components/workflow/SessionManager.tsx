...
export const SessionManager = ({ open, onOpenChange }: SessionManagerProps) => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const sessionStats = {
        ...
        totalStorageUsed: sessionList.reduce((total, s) => total + (s.nodeCount + s.edgeCount) * 0.1, 0) // Rough estimate
      };
  ...
  const handleDuplicateSession = async (sessionId: string) => {
    try {
      ...
      toast.error(`Failed to duplicate session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleImportSession = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(`Failed to import session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
          ...
          {stats && (
            <div className="mb-4 p-4 bg-muted/50 rounded-lg">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">{formatStorageSize(stats.totalStorageUsed)}</div>
