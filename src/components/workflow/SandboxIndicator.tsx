...
export const SandboxIndicator = ({ className = '' }: SandboxIndicatorProps) => {
  ...
  const getResourceStatus = () => {
    ...
    const { totalExecutions, successfulExecutions, failedExecutions, resourceLimitViolations } = sandboxStats;
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <Badge 
            variant={resourceStatus.status === 'success' ? 'default' : 
                    resourceStatus.status === 'warning' ? 'secondary' : 
                    resourceStatus.status === 'error' ? 'destructive' : 'secondary'}
            className="text-xs"
      ...
      <PopoverContent className="w-72 sm:w-80 max-w-[90vw]" align="end">
        <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-green-600">{sandboxStats.successfulExecutions}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Failed:</span>
                    <span className="font-medium text-red-600">{sandboxStats.failedExecutions}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Avg. Time:</span>
                    <span className="font-medium">{formatDuration(sandboxStats.averageExecutionTime)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Resource Violations:</span>
                    <span className="font-medium text-orange-600">{sandboxStats.resourceLimitViolations}</span>
                  </div>
                </div>
              </div>
            </>
          ...
          <div className="space-y-2">
            <h4 className="font-medium text-sm flex items-center gap-2">
              {resourceStatus.status === 'success' ? <CheckCircle className="w-4 h-4 text-green-600" /> :
               resourceStatus.status === 'warning' ? <AlertTriangle className="w-4 h-4 text-orange-600" /> :
               resourceStatus.status === 'error' ? <AlertTriangle className="w-4 h-4 text-red-600" /> :
