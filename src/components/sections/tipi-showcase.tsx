...
export const TipiShowcase = () => {
  ...
  const [infoText, setInfoText] = useState(
    "Menampilkan USR-G771-E LTE Modem di dalam TIPI gateway. Klik tombol di bawah untuk berinteraksi."
  );
  useEffect(() => {
  ...
  const toggleIntegration = () => {
    ...
    if (newState) {
      ...
      setInfoText(
        "Produk TIPI lengkap dipasang di belakang layar timbangan, terhubung untuk solusi IoT yang sempurna."
      );
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-foreground-muted text-lg max-w-3xl mx-auto">
            Jelajahi fitur dan kemampuan integrasi TIPI IoT Gateway kami. Klik tombol di bawah untuk
            melihatnya beraksi.
          </p>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-12">
            <div className="bg-gradient-to-br from-brand-primary to-brand-primary/80 p-6 rounded-2xl shadow-elegant border border-brand-primary/20 hover:shadow-glow transition-all duration-300">
              ...
              <p className="text-white/90">
                USR-G771-E LTE Modem untuk koneksi internet yang andal
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-brand-primary to-brand-primary/80 p-6 rounded-2xl shadow-elegant border border-brand-primary/20 hover:shadow-glow transition-all duration-300">
              ...
              <p className="text-white/90">
                Port RS232/RS485 untuk koneksi dengan berbagai perangkat
              </p>
            </div>
...
function createPackagingBox(
  ...
  const bottom = new THREE.Mesh(
    new THREE.BoxGeometry(boxDims.width, boxThickness, boxDims.depth),
    boxMaterial
  );
  bottom.position.y = -boxDims.height / 2;
  const front = new THREE.Mesh(
    new THREE.BoxGeometry(boxDims.width, boxDims.height, boxThickness),
    boxMaterial
  );
  ...
  const left = new THREE.Mesh(
    new THREE.BoxGeometry(boxThickness, boxDims.height, boxDims.depth),
    boxMaterial
  );
