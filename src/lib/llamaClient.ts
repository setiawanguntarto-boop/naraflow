export async function generateWorkflowFromPrompt(endpoint: string, apiKey?: string, prompt: string, mode: 'local' | 'cloud' = 'local') {
...
export async function detectLocalLlama(endpoint: string = 'http://localhost:11434'): Promise<boolean> {
  try {
...
export async function pingLlama(endpoint: string, apiKey?: string, mode: 'local' | 'cloud' = 'local') {
