...
export const broilerConfig = {
  ...
  dailyLogFields: [
    ...
    { key: 'mortality', label: 'Mortalitas', type: 'number', required: true, validation: { min: 0 } },
    ...
    { key: 'avg_weight_g', label: 'Berat Rata-rata (g)', type: 'number', required: true, validation: { min: 0 } },
...
export function getFieldMapping(key: string): BroilerFieldMapping | undefined {
  return [...broilerConfig.dailyLogFields, ...broilerConfig.harvestLogFields].find(field => field.key === key);
}

/**
 * Validate field value according to mapping rules
 */
export function validateFieldValue(field: BroilerFieldMapping, value: any): { valid: boolean; error?: string } {
