...
vi.mock('@/lib/nodeTypeRegistry', () => ({
  nodeTypeRegistry: {
    getNodeType: vi.fn((nodeType: string) => {
      ...
      const knownTypes = ['whatsapp.trigger', 'ai.chatModel', 'memory.set', 'whatsapp.send', 'validation.basic'];
  ...
  describe('validateWorkflow', () => {
    it('should validate a valid workflow with no errors or warnings', () => {
      const workflow: WorkflowOutput = {
        nodes: [
          { id: 'node-1', type: 'whatsapp.trigger', position: { x: 100, y: 300 }, data: { label: 'Trigger' } },
          { id: 'node-2', type: 'ai.chatModel', position: { x: 350, y: 300 }, data: { label: 'Chat' } }
        ],
        edges: [
          { id: 'e-1-2', source: 'node-1', target: 'node-2', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } }
        ],
        metadata: {
          title: 'Test Workflow',
          description: 'Test',
    ...
    it('should detect invalid edge connections and generate errors', () => {
      const workflow: WorkflowOutput = {
        ...
        edges: [
          { id: 'e-invalid', source: 'node-1', target: 'nonexistent-node', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } }
        ],
        metadata: {
          title: 'Test Workflow',
          description: 'Test',
    ...
    it('should warn about orphaned nodes (not connected)', () => {
      const workflow: WorkflowOutput = {
        nodes: [
          { id: 'node-1', type: 'whatsapp.trigger', position: { x: 100, y: 300 }, data: { label: 'Trigger' } },
          { id: 'node-2', type: 'ai.chatModel', position: { x: 350, y: 300 }, data: { label: 'Chat' } },
          { id: 'orphan', type: 'memory.set', position: { x: 600, y: 300 }, data: { label: 'Orphan' } }
        ],
        edges: [
          { id: 'e-1-2', source: 'node-1', target: 'node-2', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } }
        ],
    ...
    it('should not warn about single node workflows', () => {
      const workflow: WorkflowOutput = {
        nodes: [
          { id: 'node-1', type: 'default', position: { x: 100, y: 300 }, data: { label: 'Single' } }
        ],
        edges: [],
        metadata: {
          title: 'Test Workflow',
    ...
    it('should detect cycles in workflow graph', () => {
      const workflow: WorkflowOutput = {
        nodes: [
          { id: 'node-1', type: 'whatsapp.trigger', position: { x: 100, y: 300 }, data: { label: 'A' } },
          { id: 'node-2', type: 'ai.chatModel', position: { x: 350, y: 300 }, data: { label: 'B' } }
        ],
        edges: [
          { id: 'e-1-2', source: 'node-1', target: 'node-2', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } },
          { id: 'e-2-1', source: 'node-2', target: 'node-1', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } }
        ],
        metadata: {
          title: 'Test Workflow',
          description: 'Test',
    ...
    it('should validate complex workflow with multiple node types', () => {
      const workflow: WorkflowOutput = {
        nodes: [
          ...
          { id: 'trigger', type: 'whatsapp.trigger', position: { x: 350, y: 300 }, data: { label: 'Trigger' } },
          { id: 'chat', type: 'ai.chatModel', position: { x: 600, y: 300 }, data: { label: 'Chat' } },
          { id: 'memory', type: 'memory.set', position: { x: 850, y: 300 }, data: { label: 'Memory' } },
          { id: 'send', type: 'whatsapp.send', position: { x: 1100, y: 300 }, data: { label: 'Send' } },
          { id: 'end', type: 'default', position: { x: 1350, y: 300 }, data: { label: 'End' } }
        ],
        edges: [
          { id: 'e-1', source: 'start', target: 'trigger', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } },
          { id: 'e-2', source: 'trigger', target: 'chat', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } },
          { id: 'e-3', source: 'chat', target: 'memory', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } },
          { id: 'e-4', source: 'memory', target: 'send', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } },
          { id: 'e-5', source: 'send', target: 'end', sourceHandle: 'default', targetHandle: 'default', type: 'smoothstep', data: { label: null } }
        ],
        metadata: {
