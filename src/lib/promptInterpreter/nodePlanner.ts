...
export function planNodes(analysis: PromptAnalysis): NodePlan[] {
  ...
  plans.push({
    ...
    config: {
      ...
      promptTemplate:
        "Extract data from the WhatsApp message:\n\n{{payload.message}}\n\nReturn only a valid JSON object.",
      temperature: 0.1,
      maxTokens: 500,
      outputSchema: {
        type: "object",
        properties: analysis.entities.reduce(
          (acc, entity) => {
...
function generateSystemPrompt(analysis: PromptAnalysis): string {
  const fieldsList = analysis.entities
    .map(
      entity => `- ${entity.field}: ${entity.type} (required: ${entity.required ? "yes" : "no"})`
    )
    .join("\n");
