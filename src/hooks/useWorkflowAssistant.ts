...
export function useWorkflowAssistant() {
          ...
          if (result.success && result.workflow) {
            ...
            setMessages(prev => [
              ...prev,
              {
                type: "assistant",
                content:
                  result.error || "Failed to generate workflow. Please provide more details.",
                timestamp: new Date(),
              },
            ]);
          }
  ...
  const applyWorkflow = useCallback(
    (workflow: any) => {
      ...
      const nodesRecord = Object.fromEntries(workflow.nodes.map((node: any) => [node.id, node]));
      const edgesRecord = Object.fromEntries(workflow.edges.map((edge: any) => [edge.id, edge]));
