...
export class ConnectionLabelValidator {
  ...
  static validateConnection(
    ...
    for (const rule of applicableRules) {
      ...
      if (rule.disallowedLabels?.includes(labelId)) {
        errors.push(
          `Label "${labelId}" is not allowed for connection: ${fromNodeType} → ${toNodeType}`
        );
      }
      ...
      if (rule.allowedLabels && !rule.allowedLabels.includes(labelId)) {
        errors.push(
          `Label "${labelId}" is not valid for connection: ${fromNodeType} → ${toNodeType}`
        );
      ...
      if (rule.requiredLabels && !rule.requiredLabels.includes(labelId)) {
        warnings.push(
          `Consider using one of the recommended labels: ${rule.requiredLabels.join(", ")}`
        );
      }
    }
  ...
  private static validateSemanticConsistency(
    ...
    const semanticRules = [
      ...
      {
        condition: () =>
          labelId.includes("ai.") && !toNodeType.includes("AI") && !toNodeType.includes("Decision"),
