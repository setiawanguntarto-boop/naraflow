...
export class TemplateValidator {
  private static readonly rules: ValidationRule[] = [
    ...
    
    {
      ...
      validate: (template) => {
        const { nodes } = template;
        const hasStartNode = nodes.some(n => n.type === 'start' || String(n.data?.label || '').toLowerCase().includes('start'));
    ...
    
    {
      ...
      validate: (template) => {
        const { nodes } = template;
        const hasEndNode = nodes.some(n => n.type === 'end' || String(n.data?.label || '').toLowerCase().includes('end'));
    ...
    
    {
      ...
      validate: (template) => {
        ...
        const templateSize = (typeof Blob !== 'undefined')
          ? new Blob([json]).size
        ...
        if (templateSize > maxSize) {
          return {
            ...
            details: [`Size: ${Math.round(templateSize / 1024)}KB, Limit: ${Math.round(maxSize / 1024)}KB`]
          };
        }
  ...
  static sanitizeTemplate(template: TemplateData): TemplateData {
    ...
    sanitized.metadata.description = this.sanitizeText(String(sanitized.metadata.description || ''));
